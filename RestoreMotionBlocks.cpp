...
static const VSFrameRef* VS_CC RestoreMotionBlocksGetFrame(
    ...
    if (activationReason == arInitial) {
        ...
        const VSFrameRef* prev_frame =
            vsapi->getFrameFilter(n + d->before_offset, d->before, frameCtx);
        const VSFrameRef* restore_frame =
            vsapi->getFrameFilter(n, d->restore, frameCtx);
        const VSFrameRef* next_frame =
            vsapi->getFrameFilter(n + d->after_offset, d->after, frameCtx);

        VSFrameRef* dest_frame = vsapi->copyFrame(src_frame, core);

        if (RemoveDirtProcessFrame(&d->rd, dest_frame, restore_frame,
                                   prev_frame, next_frame, vsapi,
                                   d->vi) > d->mthreshold) {
...
void VS_CC RestoreMotionBlocksCreate(const VSMap* in, VSMap* out,
    ...
    d.mthreshold =
        (d.mthreshold * d.rd.pp.mdd.md.hblocks * d.rd.pp.mdd.md.vblocks) / 100;
    ...
    *data = d;
    ...
    vsapi->createFilter(in, out, "RestoreMotionBlocks", RestoreMotionBlocksInit,
                        RestoreMotionBlocksGetFrame, RestoreMotionBlocksFree,
