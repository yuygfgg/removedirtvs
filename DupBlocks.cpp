...
static const VSFrameRef* VS_CC DupBlocksGetFrame(
    ...
    if (activationReason == arInitial) {
        ...
        if (RemoveDirtProcessFrame(&d->rd, d->lf, restore_frame, d->lf,
                                   restore_frame, vsapi,
                                   d->vi) > d->mthreshold) {
            return restore_frame;
        }
...
void VS_CC DupBlocksCreate(const VSMap* in, VSMap* out, void* userData,
    ...
    *data = d;
    ...
    vsapi->createFilter(in, out, "DupBlocks", DupBlocksInit, DupBlocksGetFrame,
                        DupBlocksFree, fmParallel, 0, data, core);
}
